...
def table2csv(table, sep, missing, collapsed=0):
    ...
    if collapsed>0:
        ...
        formatted.append(t)
        for row in table[1:]:
            for c, col in enumerate(row[collapsed:]):
                t = row[:collapsed]                
                t.append(format_value(table[0][collapsed + c], missing, True))
                t.append(format_value(col, missing, True))
                formatted.append(t)
    else:
        formatted = [[format_value(value, missing, True) for value in row]
                  for row in table]
    return '\n'.join(sep.join(row) for row in formatted)

def table2iode(table, sample, dimcount, sep, missing):
    '''table is a list of lists'''
    if not table:
        return ''
    numcol = max(len(row) for row in table)
    # drop first line
    table.pop(0)
    table.insert(0, ["sample %s" % sample])
    for row in table:
        if len(row) < numcol:
            row.extend([''] * (numcol - len(row)))
        row[0] = sep.join(row[0:dimcount])
        i = 1
        while i < dimcount:
            row.pop(1)
            i += 1
